...
The claim is the stronger evidence, so it is read before the record scan and consulted inside it; that ordering is what keeps a stale record and its live successor from refusing each other's cleanup forever.
The same claim answers the inverse question at session start: a slot whose claimant has no live worker is a leak the whole pool pays for, so bootstrap names each one and the exact cleanup command for it without ever returning, resetting, or re-claiming a slot, because a held slot can still carry work no branch holds ([`bin/fm-pool-leak-lib.sh`](../bin/fm-pool-leak-lib.sh)).
