...
# The 2026-09-15 deadlock: one pool slot, two task records, and a claim naming
# exactly one of them. The claim is written under the project lock at allocation
# by bin/fm-spawn.sh, so it settles which record is stale - and neither record
# may be held hostage by the other.
test_slot_claim_breaks_the_two_record_deadlock() {
  local dir stale=stale-task live=live-task worker rc second_home

  # Direction 1: tearing down the STALE record, whose slot the claim says now
  # belongs to the live task. Its own cleanup must finish without the live
  # record blocking it, and without touching the slot.
  dir=$(make_case slot-deadlock-stale)
  mark_case_as_treehouse_pool "$dir"
  ...
  set -e
  [ "$rc" -eq 0 ] \
    || fail "a live record blocked teardown of the stale record its own slot claim outranks: $(cat "$dir/stderr")"
  kill -0 "$worker" 2>/dev/null || fail "stale-record teardown killed the live worker in the claimed slot"
  assert_absent "$dir/home/state/$stale.meta" "stale-record teardown left its own record"
  assert_present "$dir/home/state/$live.meta" "stale-record teardown removed the live task's record"
  assert_present "$dir/worktree/sentinel" "stale-record teardown reset the claimed slot"
  assert_reassigned_slot_left_alone "$dir" "$stale" "$live" "two records, claim naming the live task"
  kill "$worker" 2>/dev/null || true
  wait "$worker" 2>/dev/null || true

  # Direction 2: tearing down the LIVE record, the one the claim names, while
  # the stale record still names the same slot. The claim proves the other
  # record lost the slot, so it must not refuse this task's own return either.
  dir=$(make_case slot-deadlock-live)
  mark_case_as_treehouse_pool "$dir"
  fm_write_meta "$dir/home/state/$stale.meta" \
    "window=firstmate:fm-$stale" "endpoint_task_id=$stale" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=scout"
  ...
  set -e
  [ "$rc" -eq 0 ] \
    || fail "a stale record blocked teardown of the task its own slot claim names: $(cat "$dir/stderr")"
  assert_absent "$dir/home/state/$live.meta" "claim-holder teardown left its own record"
  assert_present "$dir/home/state/$stale.meta" "claim-holder teardown removed the stale record it does not own"
  assert_absent "$dir/pool/1/.fm-slot-owner" "claim-holder teardown left its spent slot claim behind"
  grep -Fq "treehouse <return>" "$dir/runtime.log" \
    || fail "claim-holder teardown did not return its own pool slot: $(cat "$dir/runtime.log")"
  assert_contains "$(cat "$dir/stderr")" "$stale" \
    "the claim-holder's warning should name the stale record it overrode"

  # A record whose id matches the claim cannot be told apart from this record by
  # the claim alone, so an identically named record in another home still refuses.
  dir=$(make_case slot-deadlock-same-id)
  mark_case_as_treehouse_pool "$dir"
  printf 'fixture\n' > "$dir/project/tracked"
  git -C "$dir/project" add tracked
  git -C "$dir/project" -c user.name=test -c user.email=test@example.invalid commit -qm fixture
  second_home="$dir/secondmate-home"
  mkdir -p "$second_home/projects" "$second_home/state" "$second_home/data"
  git clone -q "$dir/project" "$second_home/projects/project"
  printf '%s\n' "- mate - fixture (home: $second_home; scope: test; projects: project; added 2026-01-01)" \
    > "$dir/home/data/secondmates.md"
  fm_write_meta "$dir/home/state/$live.meta" \
    "window=firstmate:fm-$live" "endpoint_task_id=$live" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=scout"
  fm_write_meta "$second_home/state/$live.meta" \
    "window=firstmate:fm-$live" "endpoint_task_id=$live" \
    "worktree=$dir/worktree" "project=$second_home/projects/project" "kind=scout"
  claim_pool_slot "$dir" "$live"
  ...
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "a slot claim resolved a collision between two records of the same task id"
  assert_present "$dir/worktree/sentinel" "same-id collision reset the shared slot"
  assert_present "$dir/home/state/$live.meta" "same-id collision removed the task record"
  [ ! -s "$dir/runtime.log" ] \
  ...
  # A colliding record whose slot carries no claim at all proves nothing: no
  # claim says which of the two records took the slot last, which is the state of
  # every slot handed out before claims existed. It refuses.
